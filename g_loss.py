...
def loss_gaussian(feature_no,batch_size):
    ...
    def loss_split(yTrue, yPred):
        return npairs_loss(K.tf.boolean_mask(yTrue, np.array([True, False] * (batch_size/2))),
                           K.tf.boolean_mask(yPred, np.array([True, False] * (batch_size/2))),
                           K.tf.boolean_mask(yPred, np.array([False, True] * (batch_size/2)))) 
    ...
    def npairs_loss(labels, embeddings_anchor, embeddings_positive, reg_lambda=0.002):
        ...
        embeddings_anchor = K.tf.reshape(embeddings_anchor, [(batch_size/2), -1, feature_no])
        embeddings_positive = K.tf.reshape(embeddings_positive, [(batch_size/2), -1, feature_no])
